...
DEFAULT_EXCLUDES=("README.md" "LICENSE" ".editorconfig" "*.md")
...
EOF
    ...
    (cd "$repo_path" && tree -L 3 --charset=ascii -P "*" --dirsfirst -I ".git" 2>/dev/null || find . -type d | sort | sed -e "s/[^-][^\/]*\//  |/g" -e "s/|\([^ ]\)/|-\1/") >> "$output_file"
